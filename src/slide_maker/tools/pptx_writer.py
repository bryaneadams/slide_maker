...
from slide_maker.errors import RenderingError
from slide_maker.models import Slide, SlideDeck
...
def _presentation(template_path: str | Path | None):
    try:
        ...
        raise RenderingError(
            "python-pptx is required to render PowerPoint files. "
...
def _clear_default_slides(presentation) -> None:
    slide_ids = list(presentation.slides._sldIdLst)  # noqa: SLF001 - python-pptx has no public delete API.
...
def _add_image(pptx_slide, image_path: Path, warnings: list[str]) -> None:
    ...
    try:
        ...
        warnings.append(f"Image could not be added and was skipped: {image_path} ({exc})")
